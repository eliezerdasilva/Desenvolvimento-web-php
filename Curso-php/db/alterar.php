...
if($_GET['codigo']) {
    ...
    if($stmt->execute()) {
        ...
        if($resultado->num_rows > 0) {
            ...
            if($dados['salari']) {
                $dados['salari'] = str_replace(".", ",", $dados['salari']);
            }
        }
    }
}
...
if(count($_POST) > 0) {
    ...
    if (!filter_var($dados['salari'],
        FILTER_VALIDATE_FLOAT, $salarioConfig)) {
        $erros['salari'] = 'Salário inválido';
    }
    ...
    if(!count($erros)) {
        ...
        $params = [
            ...
            $dados['site'],
            $dados['filhos'],
            $dados['salari'] ? str_replace(",", ".", $dados['salari']) : null,
            $dados['id'],
        ];
...
<form action="#" method="post">
    ...
    <div class="form-row">
        ...
        <div class="form-group col-md-6">
            <label for="salari">Salário</label>
            <input type="text"
                class="form-control <?= $erros['salari'] ? 'is-invalid' : ''?>"
                id="salari" name="salari"
                placeholder="Salário"
                value="<?= $dados['salari'] ?>">
            <div class="invalid-feedback">
                <?= $erros['salari'] ?>
